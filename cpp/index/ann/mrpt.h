...
public:
  ...
  double gap_width; // The gap between the query projection and split value at
                    // the parent of node
};
...
public:
   ...
   * @param density_ - Expected ratio of non-zero components in a projection
   * matrix.
   */
  Mrpt(const Map<MatrixXd> *X_, int n_trees_, int depth_, float density_)
      : X(X_), n_samples(X_->cols()), dim(X_->rows()), n_trees(n_trees_),
        depth(depth_), density(density_), n_pool(n_trees_ * depth_),
        n_array(1 << (depth_ + 1)) {}
  ...
  void grow() {
    ...
    split_points = MatrixXd(n_array, n_trees);
...
#pragma omp parallel for
    for (int n_tree = 0; n_tree < n_trees; n_tree++) {
      ...
      std::vector<VectorXi> t =
          grow_subtree(indices, 0, 0, n_tree, tree_projections);
   ...
   * interprets each index object in leaves returned by tree traversals as
   * votes, and only performs the final linear search with the 'elect' most
   * voted objects. The priority queue trick keeps track of nodes where the
   * split value was close to the projection so that we can split the tree
   * traversal to both subtrees if we want.
   ...
   * @param votes_required - The number of votes required for an object to be
   * included in the linear search step
   * @param branches - The number of extra branches explored in the priority
   * queue trick
   * @param out - The output buffer
   * @return
   */
  vector<int> query(const Map<VectorXd> &q, int k, int votes_required,
                    int branches) const {
    VectorXd projected_query(n_pool);
    if (density < 1)
    ...
    for (int n_tree = 0; n_tree < n_trees; ++n_tree) {
      ...
      for (int d = 0; d < depth; ++d) {
        ...
        const double split_point = split_points(idx_tree, n_tree);
   ...
   * @param indices - indices of the points in the original matrix where the
   * search is made
   * @param out - output buffer
   * @return
   */
  vector<int> exact_knn(const Map<VectorXd> &q, int k, const VectorXi &indices,
                        int n_elected) const {
    vector<int> results(k);
    VectorXd distances(n_elected);
    ...
    std::partial_sort(
        idx.data(), idx.data() + mmin, idx.data() + n_elected,
        [&distances](int i1, int i2) { return distances(i1) > distances(i2); });
    for (int i = 0; i < mmin; ++i)
      results[i] = indices(idx(i));
    return results;
  }
...
private:
   ...
   * @param tree_projections - Precalculated projection values for the current
   * tree
   * @return The leaves as a vector of VectorXis
   */
  std::vector<VectorXi> grow_subtree(const VectorXi &indices, int tree_level,
                                     int i, int n_tree,
                                     const MatrixXd &tree_projections) {
    int n = indices.size();
    int idx_left = 2 * i + 1;
    ...
    std::sort(ordered.data(), ordered.data() + ordered.size(),
              [&projections](int i1, int i2) {
                return projections(i1) < projections(i2);
              });
    ...
    split_points(i, n_tree) =
        (n % 2)
            ? projections(idx_split_point)
            : (projections(idx_split_point) + projections(idx_split_point2)) /
                  2;
    ...
    std::vector<VectorXi> v = grow_subtree(left_elems, tree_level + 1, idx_left,
                                           n_tree, tree_projections);
    std::vector<VectorXi> w = grow_subtree(right_elems, tree_level + 1,
                                           idx_right, n_tree, tree_projections);
  ...
  void build_sparse_random_matrix() {
    sparse_random_matrix = SparseMatrix<double, RowMajor>(n_pool, dim);
    ...
    for (int j = 0; j < n_pool; ++j) {
      for (int i = 0; i < dim; ++i) {
        if (uni_dist(gen) > density)
          continue;
        double value = norm_dist(gen);
        triplets.push_back(Triplet<double>(j, i, value));
      }
    }
  ...
  void build_dense_random_matrix() {
    dense_random_matrix = MatrixXd(n_pool, dim);
    ...
    std::generate(dense_random_matrix.data(),
                  dense_random_matrix.data() + n_pool * dim,
                  [&normal_dist, &gen] { return normal_dist(gen); });
  }

  const Map<MatrixXd> *X; // the data matrix
  MatrixXd split_points;  // all split points in all trees
  std::vector<std::vector<VectorXi>>
      tree_leaves; // contains all leaves of all trees,
                   // indexed as tree_leaves[tree number][leaf number][index in
                   // leaf]
  MatrixXd dense_random_matrix; // random vectors needed for all the RP-trees
  ...
  const float
      density; // expected ratio of non-zero components in a projection matrix
